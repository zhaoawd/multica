...
[飞书（Lark）集成](/lark-integration) 把 Multica 事件分流到团队群和个人私聊。四个 `LARK_*` 变量必须全部配上，否则集成静默禁用。

| 环境变量 | 默认值 | 说明 |
|---|---|---|
| `LARK_APP_ID` | 空 | 飞书应用的 App ID（`cli_xxxxx` 格式），从应用「凭证与基础信息」拿 |
| `LARK_APP_SECRET` | 空 | 飞书应用的 App Secret |
| `LARK_VERIFICATION_TOKEN` | 空 | 事件订阅页面的 Verification Token，用于校验入站 webhook 和 OAuth state HMAC |
| `LARK_ENCRYPT_KEY` | 空 | 事件订阅页面的 Encrypt Key，用于解密入站事件载荷和加密保存的 bot token |
| `LARK_CALLBACK_MODE` | `webhook` | 回调模式：`websocket`（长连接，无需公网）或 `webhook`（默认，需要公网回调地址）|

**四个核心变量任一未配时：** Settings 里的 Lark 配置 UI 显示「未配置」，事件监听不触发，notifier 全部 no-op——零开销。

完整配置流程见 [飞书集成 → Self-Host 配置](/lark-integration#self-host-配置)。
